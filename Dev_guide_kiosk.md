...
- 기기가 여러장소로 이동함
- SSL
- SecretKey (ex 하드웨어 고유넘버 + 관리자 직번)
- 토큰

## 관리자모드 진입
- 초기화면에서 특정위치 터치 후  관리자 직원신분증 QR코드 입력
- 별도 파라메터와 함께 실행하면 터치만으로 관리자모드로 진입
- 관리자가 할 일: 기기 설정 리로드, 프로그램 업데이트 등

## 프로그램업데이트
- 버그발생시 프로그램업데이트는 Kiosk API서버에서 주고받는 방식 검토
- 아이디/패스워드 등 인증정보가 하드코딩 되지 않도록 작업
...
- 모금프로그램 설정
  - api 방식 통신
  - 설정값 예시 http://168.115.33.95/b1BjW55p/donate/program/create
  - 기부프로그램 이미지는 public url을 홈페이지가 전달 해 줄 예정
...
### 결제
- 키오스크에서 기부 결제가 완료되면 키오스크 결제번호가 생성되어야 합니다. (ex: KC202100001)
- 결제 승인번호는 약정시 생성된 키오스크 약정번호에 종속됩니다.
### 전자서명
- 사용자가 작성한 전자서명파일은 sign_키오스크약정번호.png 와 같은 형태로 저장하여
